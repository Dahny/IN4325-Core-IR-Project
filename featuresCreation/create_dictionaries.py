...
def create_dictionaries_from_wiki_tables(input_folder):
        ...
        for file in files:
            with open(root + '/' + file) as json_file:
                ...
                for table_id, wiki_table in wiki_tables.items():
                    ...
                    extract_entity_and_add_to_dict(wiki_table['pgTitle'], table_id)
                    ...
                    extract_entity_and_add_to_dict(wiki_table['secondTitle'], table_id)
                    ...
                    extract_entity_and_add_to_dict(wiki_table['caption'], table_id)
                    ...
                    list(map(lambda x: extract_entity_and_add_to_dict(x, table_id), wiki_table['title']))
                    ...
                    preprocessed_data = list(map(lambda x: list(map(lambda y: preprocess_string(y), x)),
                                                 wiki_table['data']))
                    list(map(lambda x: list(map(lambda y: extract_entity_and_add_to_dict(y, table_id), x)),
                             wiki_table['data']))
                    list(map(lambda x: list(map(lambda y: list(map(lambda z: add_to_dict(dict_data, z, table_id),
                                                                   y)), x)), preprocessed_data))
                    counter += 1
                    if counter % 100 == 0:
                        print("number of tables processed ", counter)

    print("counters ", counters)

    write_dictionary_to_file(dict_headers, main_path_name + '/dictionaries/dictionary_headers.json')
    write_dictionary_to_file(dict_page_titles, main_path_name + '/dictionaries/dictionary_page_titles.json')
    write_dictionary_to_file(dict_section_titles, main_path_name + '/dictionaries/dictionary_section_titles.json')
    write_dictionary_to_file(dict_captions, main_path_name + '/dictionaries/dictionary_captions.json')
    write_dictionary_to_file(dict_data, main_path_name + '/dictionaries/dictionary_data.json')
    write_dictionary_to_file(dict_entity_tables, main_path_name + '/dictionaries/dictionary_entity_tables.json')
    write_dictionary_to_file(dict_table_entities, main_path_name + '/dictionaries/dictionary_table_entities.json')
    write_dictionary_to_file(dict_word_entity, main_path_name + '/dictionaries/dictionary_word_entity.json')


def create_dictionaries_from_queries(queries):
    dict_queries = {}
    ...
    write_dictionary_to_file(dict_queries, main_path_name + '/dictionaries/dictionary_queries.json')
